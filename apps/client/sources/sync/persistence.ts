...
import {
    ...
    SettingsSchema,
    sanitizeCodexPermissionSettings,
} from './settings';
...
import { resolveCodexRemotePermissionMode } from './agentDefaults';

const mmkv = new MMKV({ id: frontendPreviewEnabled ? 'codex-plus-preview' : 'codex-plus' });
...
export function loadPendingSettings(): Partial<Settings> {
    ...
    if (pending) {
        try {
            ...
            return sanitizeCodexPermissionSettings(SettingsSchema.partial().parse(parsed));
        } catch (e) {
...
export function savePendingSettings(settings: Partial<Settings>) {
    mmkv.set('pending-settings', JSON.stringify(sanitizeCodexPermissionSettings(settings)));
}
...
export function loadNewSessionDraft(): NewSessionDraft | null {
    ...
    try {
        ...
        const wasCodexDraft = parsed.agentType === 'codex';
        const agentType: NewSessionAgentType = 'codex';
        const permissionMode: PermissionModeKey | null = wasCodexDraft && typeof parsed.permissionMode === 'string'
            ? resolveCodexRemotePermissionMode(parsed.permissionMode)
            : null;
        const modelMode: string | null = wasCodexDraft && typeof parsed.modelMode === 'string' ? parsed.modelMode : null;
        const effortLevel: string | null = wasCodexDraft && typeof parsed.effortLevel === 'string' ? parsed.effortLevel : null;
        const sessionType: NewSessionSessionType = 'simple';
        const worktreeKey = null;
...
export function saveNewSessionDraft(draft: NewSessionDraft) {
    mmkv.set(NEW_SESSION_DRAFT_KEY, JSON.stringify({
        ...draft,
        agentType: 'codex',
        permissionMode: draft.permissionMode === null
            ? null
            : resolveCodexRemotePermissionMode(draft.permissionMode),
        sessionType: 'simple',
        worktreeKey: null,
    }));
}
