...
describe('settings', () => {
    ...
    describe('settingsToSyncPayload', () => {
        ...
        it('fails legacy Codex permission settings closed before syncing', () => {
            const settings = {
                ...settingsDefaults,
                lastUsedPermissionMode: 'yolo',
                agentDefaultOverrides: {
                    codex: { permissionMode: 'safe-yolo', modelMode: 'gpt-5.6-sol' },
                },
            } as Settings;

            expect(settingsToSyncPayload(settings)).toMatchObject({
                lastUsedPermissionMode: 'read-only',
                agentDefaultOverrides: {
                    codex: { permissionMode: 'read-only', modelMode: 'gpt-5.6-sol' },
                },
            });
        });
    });

    it('migrates legacy Codex permission settings while parsing', () => {
        const parsed = settingsParse({
            lastUsedPermissionMode: 'yolo',
            agentDefaultOverrides: {
                codex: { permissionMode: 'safe-yolo' },
            },
        });

        expect(parsed.lastUsedPermissionMode).toBe('read-only');
        expect(parsed.agentDefaultOverrides.codex?.permissionMode).toBe('read-only');
    });

    it('sanitizes pending partial settings without filling unrelated defaults', () => {
        expect(sanitizeCodexPermissionSettings({
            lastUsedPermissionMode: 'safe-yolo',
            agentDefaultOverrides: {
