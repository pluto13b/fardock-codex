...
class Sync {
    ...
    constructor() {
        ...
        AppState.addEventListener('change', (nextAppState) => {
            ...
            if (!this.credentials || frontendPreviewEnabled) {
                return;
            }
    ...
    async #init() {
        ...
        this.machinesSync.invalidate();
        log.log('🔄 #init: Codex task, settings, and machine syncs invalidated');
        ...
        try {
            await Notifications.scheduleNotificationAsync({
                content: {
                    ...
                    body: 'A message failed to send while the app was in background. Open Codex Plus and retry.',
                    sound: true
                },
                trigger: null
            });
     ...
     * Returns every successful upload plus a failure count. The caller must
     * discard the whole batch when any failure is reported.
     */
    private async uploadAttachmentsForSession(
        ...
        for (const attachment of attachments) {
            try {
                ...
                failed++;
                // Keep checking the batch so diagnostics account for every
                // selected image. sendMessage discards all successes if any
                // upload failed.
            }
        }
    ...
    async sendMessage(sessionId: string, text: string, options?: SendMessageOptions): Promise<boolean> {
        ...
        if (!encryption) {
            ...
            if (!encryption) {
                throw new Error('The task is not ready yet. Please reconnect and retry.');
            }
        }
        ...
        if (!session) {
            ...
            if (!session) {
                throw new Error('The task is no longer available. Refresh the task list and retry.');
            }
        }
        ...
        if (effectiveAttachments && effectiveAttachments.length > 0) {
            ...
            const uploadBatch = resolveAttachmentUploadBatch(
            ...
            if (!uploadBatch.complete) {
                Modal.alert(
                    ...
                    t('imageUpload.uploadFailedMessage', { count: uploadBatch.failureCount }),
                    [{ text: t('common.ok'), style: 'cancel' }],
                );
                return false;
            }
        ...
        // An attachment-only action is represented by its file records. Do not
        // enqueue an empty text record, and do not clear the picker when every
        // upload failed.
        if (!text.trim()) {
