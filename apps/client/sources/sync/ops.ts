...
export async function machineSpawnNewSession(options: SpawnSessionOptions): Promise<SpawnSessionResult> {
    ...
    // Final outbound guard: UI state, defaults, or an imported draft can never
    // turn a Codex spawn into an elevated remote session.
    const explicitNonCodexAgent = agent === 'claude'
        || agent === 'gemini'
        || agent === 'openclaw'
        || agent === 'agy'
        || agent === 'rig';
    const guardAsCodex = agent === 'codex' || Boolean(resumeCodexThreadId) || !explicitNonCodexAgent;
    const outboundPermissionMode = guardAsCodex
    ...
    try {
        ...
        const request: SpawnRequest = agent === 'rig'
            ? {
                ...
                ...(outboundPermissionMode ? { permissionMode: outboundPermissionMode } : {}),
                ...(providerId ? { providerId } : {}),
                ...(modelId ? { modelId } : {}),
                ...((effort ?? effortLevel) ? { effort: effort ?? effortLevel } : {}),
            }
            : { type: 'spawn-in-directory', directory, approvedNewDirectoryCreation, token, agent, permissionMode: outboundPermissionMode, modelMode, effortLevel, resumeClaudeSessionId, resumeCodexThreadId, parentSessionId, forkedFromMessageId, isSideChat };
...
export async function machineResumeSession(options: ResumeSessionOptions & { model?: string; permissionMode?: string }): Promise<SpawnSessionResult> {
    ...
    const outboundPermissionMode = shouldGuardCodexRemotePermissions(session?.metadata)
        ? resolveCodexRemotePermissionMode(permissionMode)
        : permissionMode;
    ...
    try {
        const result = await apiSocket.machineRPC<SpawnSessionResult, { sessionId: string; model?: string; permissionMode?: string }>(
            ...
            { sessionId, model, permissionMode: outboundPermissionMode },
        );
        return result;
    } catch (error) {
        return {
...
export function sessionSetAgentModes(sessionId: string, patch: SessionAgentModesPatch): void {
    ...
    const guardedPatch: SessionAgentModesPatch = {
        ...patch,
        ...(patch.permissionMode !== undefined && patch.permissionMode !== null
            && shouldGuardCodexRemotePermissions(session?.metadata)
            ? { permissionMode: resolveCodexRemotePermissionMode(patch.permissionMode) }
            : {}),
    };
    ...
    if (guardedPatch.permissionMode !== undefined && isChanged(guardedPatch.permissionMode, 'permissionMode')) {
        changed.permissionMode = guardedPatch.permissionMode;
    }
    if (guardedPatch.modelMode !== undefined && isChanged(guardedPatch.modelMode, 'modelMode')) {
        changed.modelMode = guardedPatch.modelMode;
    }
    if (guardedPatch.effortLevel !== undefined && isChanged(guardedPatch.effortLevel, 'effortLevel')) {
        changed.effortLevel = guardedPatch.effortLevel;
    }
...
export async function sessionAllow(sessionId: string, id: string, mode?: 'default' | 'acceptEdits' | 'bypassPermissions' | 'plan', allowedTools?: string[], decision?: 'approved' | 'approved_for_session', updatedInput?: Record<string, unknown>): Promise<void> {
    ...
    if (shouldGuardCodexRemotePermissions(metadata)) {
        if (!isExplicitCodexSessionMetadata(metadata)) {
            throw new Error('Cannot approve an ambiguous or unloaded Codex task');
        }
        if (!session?.agentState?.requests?.[id]) {
            throw new Error('Cannot approve a Codex request that is missing, completed, or belongs to another task');
        }
        const requestsElevatedScope = (mode !== undefined && mode !== 'default')
            || (allowedTools?.length ?? 0) > 0
            || decision === 'approved_for_session';
        if (requestsElevatedScope) {
            throw new Error('Remote Codex approvals must be one-time and cannot elevate task permissions');
        }
    }
