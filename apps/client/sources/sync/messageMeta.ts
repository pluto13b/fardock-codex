...
export function resolveMessageModeMeta(
    ...
    if (session.permissionMode !== null && session.permissionMode !== undefined) {
        meta.permissionMode = isCodexSession
            ? resolveCodexRemotePermissionMode(session.permissionMode)
            : session.permissionMode;
    } else if (agentOverrides.permissionMode !== undefined) {
        meta.permissionMode = isCodexSession
            ? resolveCodexRemotePermissionMode(agentOverrides.permissionMode)
            : agentOverrides.permissionMode;
    } else if (isCodexSession) {
        meta.permissionMode = resolveCodexRemotePermissionMode(undefined);
    }
