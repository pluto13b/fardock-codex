...
export function SessionsList({
    ...
    // Request review
    React.useEffect(() => {
        if (!frontendPreviewEnabled && sourceData && sourceData.length > 0) {
            requestReview();
        }
    ...
    const renderItem = React.useCallback(({ item, index }: { item: SessionListViewItem, index: number }) => {
        switch (item.type) {
            case 'header':
                return (
                    <View style={presentation === 'sidebar' ? styles.sidebarHeaderSection : styles.headerSection}>
                        <Text style={presentation === 'sidebar' ? styles.sidebarHeaderText : styles.headerText}>
                            {item.title}
                        </Text>
                    </View>
                );
            ...
            case 'projects-header':
                return (
                    <View style={presentation === 'sidebar' ? styles.sidebarHeaderSection : styles.headerSection}>
                        <Text style={presentation === 'sidebar' ? styles.sidebarHeaderText : styles.headerText}>
                            {item.source === 'rig' ? 'Rig' : t('sidebar.projects')}
                        </Text>
                    </View>
                );
    ...
    }, [selectedSessionId, data, presentation]);
...
const SessionItem = React.memo(({ session, selected, isFirst, isLast, isSingle }: {
    ...
    return (
        ...
        {Platform.OS === 'web' && !frontendPreviewEnabled && (
