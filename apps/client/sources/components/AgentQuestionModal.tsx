...
import {
    decideFrontendPreviewCommunication,
...
export function AgentQuestionModal({ pending, sessionId, visible, onClose }: AgentQuestionModalProps) {
    ...
    const handleSubmit = React.useCallback(async () => {
        ...
        setError(null);
        try {
            const answers = buildAnswers(pending.questions, drafts);
            if (frontendPreviewEnabled) {
                decideFrontendPreviewCommunication(sessionId, pending.id, 'answered', answers);
            } else {
                await sessionAnswerQuestion(sessionId, pending.id, answers, pending.kind);
            }
            onClose();
        } catch (submitError) {
    ...
    const handleDismiss = React.useCallback(async () => {
        ...
        onClose();
        if (frontendPreviewEnabled) {
            decideFrontendPreviewCommunication(sessionId, pending.id, 'cancelled');
            return;
        }
        try {
