...
import {
    decideFrontendPreviewPermission,
...
export const PermissionFooter: React.FC<PermissionFooterProps> = ({ permission, sessionId, toolName, toolInput, metadata }) => {
    ...
    // Codex Plus is Codex-only; ambiguous legacy metadata must expose only
    // one-time Codex approval controls, never the imported session-wide modes.
    const isCodex = metadata?.client?.id !== 'rig'
        && (shouldGuardCodexRemotePermissions(metadata) || toolName.startsWith('Codex'));
    const canApproveCodex = isExplicitCodexSessionMetadata(metadata);
    ...
    const handleCodexApprove = async () => {
        ...
        try {
            if (frontendPreviewEnabled) {
                decideFrontendPreviewPermission(sessionId, permission.id, 'approved');
            } else {
    ...
    const handleCodexAbort = async () => {
        ...
        try {
            if (frontendPreviewEnabled) {
                decideFrontendPreviewPermission(sessionId, permission.id, 'abort');
            } else {
            ...
            <View style={styles.container}>
                ...
                >
                    {renderPermissionButton({
                        label: getCurrentLanguage().startsWith('zh') ? '仅允许这一次' : 'Allow once',
                        loading: loadingButton === 'allow',
                        onPress: handleCodexApprove,
                        disabled: !canApproveCodex || !isPending || loadingButton !== null || loadingForSession,
                        buttonStyle: [
                            styles.button,
