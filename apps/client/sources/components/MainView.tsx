...
const HeaderRight = React.memo(({
    ...
    if (activeTab === 'settings') {
        if (!isCustomServer) {
            return mobilePresentation ? null : <View style={styles.headerButton} />;
        }
        return (
            <Pressable
...
export const MainView = React.memo(({ variant }: MainViewProps) => {
    ...
    const mobilePresentation = Platform.OS !== 'web' || !isTablet;
    const router = useRouter();
    ...
    const topContentInset = mobilePresentation
        ? safeArea.top
            + MOBILE_GLASS_HEADER_HEIGHT
            + 12
        : 0;
    const bottomContentInset = mobilePresentation
    ...
    const handleHomePromptSubmit = React.useCallback(async (): Promise<boolean> => {
        ...
        if (frontendPreviewEnabled) {
            appendFrontendPreviewMessage(FRONTEND_PREVIEW_SESSION_IDS.running, prompt, attachments);
            setHomePrompt('');
            Keyboard.dismiss();
            router.navigate(`/session/${FRONTEND_PREVIEW_SESSION_IDS.running}`);
            return true;
        }
        ...
        return started;
    }, [homePrompt, router, startHomeSession]);
    ...
    if (variant === 'sidebar') {
        ...
        return (
            <View style={styles.sidebarContentContainer}>
                <SessionsList presentation="sidebar" />
            </View>
        );
    }
    ...
    const phoneHeader = (
        <View style={[styles.phoneHeader, mobilePresentation && styles.phoneHeaderOverlay]}>
            <Header
                title={searchActive && mobilePresentation
                ...
                headerLeftGlass={mobilePresentation}
                headerBackdropVisible={headerBackdropVisible}
                headerBackdropAlwaysVisible={mobilePresentation}
    ...
    return (
        <View style={styles.phoneRoot}>
            <View style={styles.phoneContainer}>
                {!mobilePresentation && phoneHeader}
                {!mobilePresentation ? renderWebTabContent() : (
