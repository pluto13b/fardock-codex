...
describe('modelModeOptions', () => {
    ...
    it('uses Agent-provided effort labels and descriptions before fallbacks', () => {
        expect(getEffortLevelsForModel('codex', 'gpt-5.6-sol', {
            thoughtLevels: [
                { code: 'low', value: '轻度', description: null },
                { code: 'ultra', value: '极高', description: '更快消耗使用额度' },
            ],
        } as any)).toEqual([
            { key: 'low', name: '轻度', description: null },
            { key: 'ultra', name: '极高', description: '更快消耗使用额度' },
        ]);
    });

    it('localizes canonical effort labels without changing protocol keys', () => {
        expect(presentEffortLevel({ key: 'xhigh', name: 'xhigh' }, 'zh-Hans')).toEqual({
            key: 'xhigh',
            name: '极高',
            description: null,
        });
        expect(presentEffortLevel({ key: 'ultra', name: 'ultra' }, 'zh-Hans')).toEqual({
            key: 'ultra',
            name: '极高',
            description: '更快消耗使用额度',
        });
        expect(presentEffortLevel({ key: 'high', name: 'Agent high', description: 'Agent copy' }, 'zh-Hans')).toEqual({
            key: 'high',
            name: 'Agent high',
    ...
    it('builds codex model fallbacks', () => {
        ...
        expect(models[1].name).toBe('5.6 Sol');
    });
    ...
    it('uses the exact Codex model catalog reported by the Agent', () => {
    ...
    it('keeps codex permission modes hardcoded even when metadata modes exist', () => {
        ...
        expect(modes.find((mode) => mode.key === 'safe-yolo')?.disabled).toBe(true);
        expect(modes.find((mode) => mode.key === 'yolo')?.disabled).toBe(true);
    });
