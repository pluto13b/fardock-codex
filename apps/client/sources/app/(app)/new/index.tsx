...
import {
    ComposerControlMenu,
    type ComposerControlMenuSection,
} from '@/components/ComposerControlMenu';
import { getComposerChoiceMenuWidth } from '@/components/composerControlMenuLayout';
...
type ComposerControlPickerType = 'add' | 'permission' | 'model' | 'effort';
...
function getPermissionIcon(mode: PermissionMode): keyof typeof Ionicons.glyphMap {
    switch (mode.semanticKind ?? mode.key) {
        case 'read-only':
...
function NewSessionScreen() {
    ...
    const [activeComposerMenu, setActiveComposerMenu] = React.useState<ComposerControlPickerType | null>(null);
    ...
    const pendingComposerMenuRef = React.useRef<ComposerControlPickerType | null>(null);
    const composerMenuKeyboardSubscriptionRef = React.useRef<ReturnType<typeof Keyboard.addListener> | null>(null);
    const composerMenuOpenTimerRef = React.useRef<ReturnType<typeof setTimeout> | null>(null);
    const composerMenuSelectionTimerRef = React.useRef<ReturnType<typeof setTimeout> | null>(null);
    ...
    }, [clearImages, expImageUpload, selectedImages]);
    ...
    const effortLevels = React.useMemo<EffortLevel[]>(
        ...
        [selectedAgent, currentLanguage, currentModelKey, rigCreation],
    );
    ...
    const isDesktop = isRunningOnMac() || (Platform.OS === 'web' && windowWidth >= 900);
    const offlineHelpCopy = currentLanguage.startsWith('zh')
        ? '请确认 Windows Agent 正在运行，并检查电脑与 Relay 的连接。'
        : 'Make sure the Windows Agent is running and check the computer and Relay connection.';
    ...
    const cancelPendingComposerMenuOpen = React.useCallback(() => {
        pendingComposerMenuRef.current = null;
        composerMenuKeyboardSubscriptionRef.current?.remove();
        composerMenuKeyboardSubscriptionRef.current = null;
        if (composerMenuOpenTimerRef.current) {
            clearTimeout(composerMenuOpenTimerRef.current);
            composerMenuOpenTimerRef.current = null;
        }
    }, []);

    const cancelPendingComposerMenuSelection = React.useCallback(() => {
        if (composerMenuSelectionTimerRef.current) {
            clearTimeout(composerMenuSelectionTimerRef.current);
            composerMenuSelectionTimerRef.current = null;
        }
    }, []);

    const closePicker = React.useCallback(() => {
        cancelPendingPickerOpen();
        cancelPendingComposerMenuOpen();
        cancelPendingComposerMenuSelection();
        setActivePicker(null);
        setActiveComposerMenu(null);
    }, [cancelPendingComposerMenuOpen, cancelPendingComposerMenuSelection, cancelPendingPickerOpen]);

    const closeComposerMenuAfterSelection = React.useCallback(() => {
        cancelPendingComposerMenuSelection();
        composerMenuSelectionTimerRef.current = setTimeout(() => {
            composerMenuSelectionTimerRef.current = null;
            setActiveComposerMenu(null);
        }, 170);
    }, [cancelPendingComposerMenuSelection]);
    ...
    const toggleComposerMenu = React.useCallback((type: ComposerControlPickerType) => {
        if (activeComposerMenu === type || pendingComposerMenuRef.current === type) {
            closePicker();
            return;
        }
        ...
        pendingComposerMenuRef.current = type;
        const finishOpening = () => {
            const nextMenu = pendingComposerMenuRef.current;
            cancelPendingComposerMenuOpen();
            if (nextMenu) {
                setActiveComposerMenu(nextMenu);
            }
        };
        composerMenuKeyboardSubscriptionRef.current = Keyboard.addListener('keyboardDidHide', finishOpening);
        composerMenuOpenTimerRef.current = setTimeout(finishOpening, 420);
        composerInputRef.current?.blur();
        Keyboard.dismiss();
    }, [activeComposerMenu, cancelPendingComposerMenuOpen, closePicker, isDesktop]);
    ...
    const handleComposerControlSelect = React.useCallback((
        type: Exclude<ComposerControlPickerType, 'add'>,
        key: string,
    ) => {
        if (type === 'model') {
            const next = modelModes.findIndex((mode) => mode.key === key && !mode.disabled && !mode.unavailable);
            if (next >= 0) {
                setModelIndex(next);
                draft.setModelMode(modelModes[next]?.key ?? 'default');
            }
        } else if (type === 'effort') {
            const next = effortLevels.findIndex((level) => level.key === key && !level.disabled);
            if (next >= 0) {
                setEffortIndex(next);
                draft.setEffortLevel(effortLevels[next]?.key ?? key);
            }
        } else {
            const next = permissionModes.findIndex((mode) => mode.key === key && !mode.disabled);
            if (next >= 0) {
                setPermissionIndex(next);
                draft.setPermissionMode(permissionModes[next]?.key ?? 'default');
            }
        }
        closeComposerMenuAfterSelection();
    }, [closeComposerMenuAfterSelection, draft.setEffortLevel, draft.setModelMode, draft.setPermissionMode, effortLevels, modelModes, permissionModes]);

    const composerControlMenuSections = React.useMemo<ComposerControlMenuSection[]>(() => {
        switch (activeComposerMenu) {
            case 'add':
                return [{
                    key: 'add',
                    items: [{
            ...
            case 'permission':
                return [{
                    ...
                    items: permissionModes.map((mode) => ({
                        ...
                        icon: getPermissionIcon(mode),
                        selected: mode.key === currentPermission?.key,
                        disabled: mode.disabled,
                        tone: mode.key === 'yolo' || mode.key === 'bypassPermissions'
                            ? 'warning' as const
                            : 'default' as const,
                        onPress: mode.disabled
                            ? undefined
                            : () => handleComposerControlSelect('permission', mode.key),
                    })),
                }];
            case 'model':
                return [{
                    key: 'model',
                    items: modelModes.map((mode) => ({
                        key: mode.key,
                        label: mode.name,
                        description: mode.description,
                        selected: mode.key === currentModelKey,
                        disabled: mode.disabled || mode.unavailable,
                        onPress: mode.disabled || mode.unavailable
                            ? undefined
                            : () => handleComposerControlSelect('model', mode.key),
                    })),
                }];
            case 'effort':
                return [{
                    ...
                    items: effortLevels.map((level) => ({
                        ...
                        selected: level.key === currentEffort?.key,
                        disabled: level.disabled,
                        onPress: level.disabled
                            ? undefined
                            : () => handleComposerControlSelect('effort', level.key),
                    })),
                }];
            default:
                return [];
    ...
    const handleSend = React.useCallback(async (
        ...
        const previewDraft = useNewSessionDraft.getState();
        if (frontendPreviewEnabled) {
            if (previewDraft.input.trim() || previewDraft.attachments.length > 0) {
                appendFrontendPreviewMessage(
                    FRONTEND_PREVIEW_SESSION_IDS.running,
                    previewDraft.input,
                    previewDraft.attachments,
                );
            }
            previewDraft.setInput('');
            previewDraft.setAttachments([]);
            clearImages();
            navigateToSession(FRONTEND_PREVIEW_SESSION_IDS.running);
            return;
        }
        ...
        try {
            ...
            const permissionKey = resolveCodexRemotePermissionMode(currentPermission?.key);
            ...
            const spawnDirectory = absolutePath;
            ...
            const spawnOptions = rigCreation
                ...
                : {
                    ...
                    permissionMode: permissionKey ?? undefined,
            ...
            switch (result.type) {
                case 'success':
                    ...
                    // Pull the live prompt. We read via getState() so this
                    ...
                    const rawPrompt = draftState.input;
                    ...
                    // Only retire the idempotency key and draft after the whole
                    // initial message (including every attachment) is queued.
                    completeSpawnRequest();
                    draftState.setInput('');
    ...
    }, [selectedMachineId, selectedMachine, selectedPath, selectedAgent, router, navigateToSession, currentPermission?.key, currentModelKey, currentEffort?.key, effectiveAgentDefaults.permissionMode, effectiveAgentDefaults.modelMode, effectiveAgentDefaults.effortLevel, worktreeKey, rigCreation, supportsWorktree, clearImages]);
    ...
    const nativeComposerMenuMaxHeight = React.useMemo(() => {
        const headerBottom = safeArea.top + MOBILE_GLASS_HEADER_HEIGHT;
        return Math.max(
            120,
            Math.min(420, windowHeight - headerBottom - mobileComposerHeight - 34),
        );
    }, [mobileComposerHeight, safeArea.top, windowHeight]);

    const nativeComposerMenuTop = React.useMemo(() => {
        const headerBottom = safeArea.top + MOBILE_GLASS_HEADER_HEIGHT;
        const composerTop = windowHeight - safeArea.bottom - mobileComposerHeight;
        const itemCount = composerControlMenuSections.reduce(
            (count, section) => count + section.items.length,
            0,
        );
        const sectionLabelHeight = composerControlMenuSections.some((section) => section.label) ? 28 : 0;
        const estimatedHeight = Math.min(
            nativeComposerMenuMaxHeight,
            18 + sectionLabelHeight + itemCount * 58,
        );
        return Math.max(headerBottom + 12, composerTop - estimatedHeight - 10);
    }, [composerControlMenuSections, mobileComposerHeight, nativeComposerMenuMaxHeight, safeArea.bottom, safeArea.top, windowHeight]);
    ...
    const composerNode = (
        <View style={styles.composerStack}>
            {!isNativeMobile && activeComposerMenu && composerControlMenuSections.length > 0 && (
                <View style={[
                    styles.composerControlPopover,
                    activeComposerMenu === 'model' || activeComposerMenu === 'effort'
                        ? styles.composerControlPopoverRight
                        : styles.composerControlPopoverLeft,
                ]}>
                    <ComposerControlMenu
                        sections={composerControlMenuSections}
                        maxHeight={Math.min(420, Math.max(160, windowHeight - 220))}
                        presentation={activeComposerMenu === 'model' || activeComposerMenu === 'effort'
                            ? 'choice'
                            : 'default'}
                        style={activeComposerMenu === 'model' || activeComposerMenu === 'effort'
                            ? {
                                width: getComposerChoiceMenuWidth(activeComposerMenu, windowWidth),
                                alignSelf: 'flex-end',
                            }
                            : undefined}
                        accessibilityLabel={activeComposerMenu === 'add'
                            ? t('settingsFeatures.imageUpload')
                            : activeComposerMenu === 'permission'
                                ? t('agentInput.codexPermissionMode.title')
                                : activeComposerMenu === 'model'
            ...
            >
                ...
                <View style={[
                    ...
                    <BubblePressable
                        onPress={() => toggleComposerMenu('add')}
                        hitSlop={5}
                        style={(pressedState) => [
                            styles.composerRoundButton,
                            activeComposerMenu === 'add' && styles.composerControlActive,
                            pressedState.pressed && styles.composerControlPressed,
                        ]}
                        accessibilityRole="button"
                    ...
                    {showPermission && currentPermission && (
                        <BubblePressable
                            onPress={() => toggleComposerMenu('permission')}
                            hitSlop={4}
                            style={(pressedState) => [
                                styles.composerValueButton,
                                styles.composerPermissionButton,
                                activeComposerMenu === 'permission' && styles.composerControlActive,
                                pressedState.pressed && styles.composerControlPressed,
                            ]}
                            accessibilityRole="button"
                        ...
                        >
                            <Ionicons
                                name={getPermissionIcon(currentPermission)}
                                size={15}
                                color={permissionStyle?.color ?? theme.colors.textSecondary}
                            />
                            <Text
                                style={[
                                    styles.composerControlText,
                                    permissionStyle?.color ? { color: permissionStyle.color } : null,
                                ]}
                                numberOfLines={1}
                            >
                    ...
                    {showModel && currentModel && (
                        <BubblePressable
                            onPress={() => toggleComposerMenu('model')}
                            hitSlop={4}
                            style={(pressedState) => [
                                styles.composerValueButton,
                                styles.composerModelButton,
                                activeComposerMenu === 'model' && styles.composerControlActive,
                                pressedState.pressed && styles.composerControlPressed,
                            ]}
                            accessibilityRole="button"
                            accessibilityLabel={`${t('agentInput.model.title')}: ${currentModel.name}`}
                        >
                            <Text style={styles.composerControlText} numberOfLines={1}>
                                {currentModel.name}
                            </Text>
                            {windowWidth > 380 && (
                    ...
                    {showEffort && currentEffort && (
                        <BubblePressable
                            onPress={() => toggleComposerMenu('effort')}
                            hitSlop={4}
                            style={(pressedState) => [
                                styles.composerValueButton,
                                styles.composerEffortButton,
                                activeComposerMenu === 'effort' && styles.composerControlActive,
                                pressedState.pressed && styles.composerControlPressed,
                            ]}
                            accessibilityRole="button"
        ...
        >
            ...
            {sidebarLayout.showSidebar ? (
                <View style={styles.desktopShell}>
                    {Platform.OS === 'web' && (activePicker || activeComposerMenu) && (
                        <Pressable
                ...
                <View style={styles.inner}>
                    {isNativeMobile && (activePicker || activeComposerMenu) && (
                        <AnimatedClickAwayBackdrop
                            exitImmediately
                            onPress={closePicker}
            ...
            {isNativeMobile && activeComposerMenu && composerControlMenuSections.length > 0 && (
                <KeyboardStickyView
                    ...
                    style={[
                        styles.nativePickerViewportPopover,
                        { top: nativeComposerMenuTop },
                    ]}
                >
                    <ComposerControlMenu
                        sections={composerControlMenuSections}
                        maxHeight={nativeComposerMenuMaxHeight}
                        presentation={activeComposerMenu === 'model' || activeComposerMenu === 'effort'
                            ? 'choice'
                            : 'default'}
                        style={activeComposerMenu === 'model' || activeComposerMenu === 'effort'
...
const styles = StyleSheet.create((theme) => ({
    ...
    composerControlPopoverLeft: {
        left: 0,
    },
    composerControlPopoverRight: {
        right: 0,
        width: 380,
    },
    rightSidebar: {
