...
describe('useNewSessionDraft', () => {
    ...
    it('fails a persisted yolo permission closed while preserving model and effort', async () => {
        ...
        expect(useNewSessionDraft.getState().permissionMode).toBe('read-only');
        expect(useNewSessionDraft.getState().modelMode).toBe('opus');
        expect(useNewSessionDraft.getState().effortLevel).toBe('xhigh');
    });

    it('fails a newly selected yolo permission closed before persisting it', async () => {
        const { useNewSessionDraft } = await import('./useNewSessionDraft');

        useNewSessionDraft.getState().setPermissionMode('yolo');

        expect(useNewSessionDraft.getState().permissionMode).toBe('read-only');
        expect(mockPersistence.saved.at(-1)).toMatchObject({ permissionMode: 'read-only' });
    });
