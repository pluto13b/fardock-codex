...
import { resolveCodexRemotePermissionMode } from '@/sync/agentDefaults';
...
export const useNewSessionDraft = create<NewSessionDraftState>()((set, get) => ({
    ...
    agentType: 'codex',
    permissionMode: initial?.permissionMode === null || initial?.permissionMode === undefined
        ? null
        : resolveCodexRemotePermissionMode(initial.permissionMode),
    ...
    setAgentType: () => { set({ agentType: 'codex' }); persist(get()); },
    setPermissionMode: (mode) => {
        set({ permissionMode: resolveCodexRemotePermissionMode(mode) });
        persist(get());
    },
    setModelMode: (mode) => { set({ modelMode: mode }); persist(get()); },
    setEffortLevel: (level) => { set({ effortLevel: level }); persist(get()); },
    setSessionType: () => { set({ sessionType: 'simple' }); persist(get()); },
    setWorktreeKey: () => { set({ worktreeKey: null }); persist(get()); },
}));
