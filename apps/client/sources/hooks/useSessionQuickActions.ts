...
import { resolveCodexRemotePermissionMode, shouldGuardCodexRemotePermissions } from '@/sync/agentDefaults';
...
export function useSessionQuickActions(
    ...
    const [resumingSession, performResume] = useHappyAction(async () => {
        ...
        const isCodexSession = shouldGuardCodexRemotePermissions(session.metadata);
        const resumePermissionMode = isCodexSession
            ? resolveCodexRemotePermissionMode(modeMeta.permissionMode)
        ...
        switch (result.type) {
            case 'success': {
                ...
                if (resumePermissionMode) {
                    sessionSetAgentModes(result.sessionId, { permissionMode: resumePermissionMode });
                }
